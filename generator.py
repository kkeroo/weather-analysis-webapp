...
def calculate_gdd(df, degree):
    ...
    tmax_df['Temp max'] = tmax
    tmax_df['thirty'] = np.ones((tmax.shape[0])) * 30
    tmax = np.min(tmax_df, axis=1)


    tmin = df['Temp min']
    tmin_df = pd.DataFrame()
    tmin_df['Temp min'] = tmin
    tmin_df['degree'] = np.ones((tmin.shape[0])) * degree
    tmin = np.max(tmin_df, axis=1)

    gdd = ((tmax + tmin) / 2) - degree
    #gdd_df = pd.DataFrame()
    #gdd_df['gdd'] = gdd
    #gdd_df['zero'] = np.zeros((gdd.shape[0]))
    #gdd = np.max(gdd_df, axis=1)
...
def daily_stats_in_year(df, degree):
    ...
    daily_df['Gdd'] = gdd
    return daily_df, gdd_total


#data_type: monthly/daily
def write_data_to_ws(df, ws, year, data_type, gdd, degree, tempGraph, rainGraph):
    #set column width
...
def create_rain_chart(ws, num_of_rows, date_type):
    ...
    data = Reference(ws, min_col=4, min_row = 2, max_row = num_of_rows)
...
def create_temp_chart(ws, num_of_rows, date_type):
    ...
    chart.shape = 4
    chart.legend = None
    ws.add_chart(chart, "K22")   

def write_dataframe_to_ws(df,ws):
...
def crop_date_range(dataframe, start_date, end_date):
    ...
    start_day = int(date_arr[1])
    start_month = int(date_arr[0])
    start_year = int(date_arr[2]) + 2000

    date_arr = end_date.split('/')
    end_day = int(date_arr[1])
    end_month = int(date_arr[0])
    end_year = int(date_arr[2]) + 2000
...
def generate_excel_file(filename, generated_file_name, start_date, end_date, base_temp, tempGraph, rainGraph, dailyData, monthlyData):
    ...
    write_dataframe_to_ws(dataframe, ws)
    ...
    for year in years:
        ...
        daily_df, gdd = daily_stats_in_year(one_year, base_temp)
        ...
        if (monthlyData):
            ...
            write_data_to_ws(monthly_df, ws, year, 'monthly', gdd, base_temp, tempGraph, rainGraph)
